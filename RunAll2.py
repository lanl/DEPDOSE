...
#os.chdir('E:\Dropbox\Workspaces\Python\Python3\DCAL-KDEP')
BaseDir = os.getcwd()
...
CaseParamsHeader = CaseParamsLines[:9]
CaseParamsLines = CaseParamsLines[9:]
...
#    with open('input.csv', 'w',newline='') as f: #Python 3 Version
    ...
    for nuclide,solubility in zip(nuclides,solubilities):
        ...
        CED = []
        data = []
        Columns = ["AMAD","AMTD","Nuclide","Subject","Activity","Solubility","f1","CED","Gonads","R-Marrow","Colon","Lungs","Stomach","Bladder","Breasts","Liver","Oesophagus","Thyroid","Skin","Bone Surface","Muscle","Brain","SI-Wall","Kidneys","Pancreas","Spleen","Thymus","Uterus","Adrenals","Extrathoracic Tissues","Remainder"]
        ...
        for AMAD,AMTD,ET1,ET2,bbe_GEL,bbe_SEQ,bbe_SOL,BBi_SEQ,BBi_SOL,BBi_GEL,AI in zip(AMADs,AMTDs,ET1s,ET2s,bbe_GELs,bbe_SEQs,bbe_SOLs,BBi_SEQs,BBi_SOLs,BBi_GELs,AIs):
            ...
            main_tissues = np.array([gonads,float(dfSumEq.loc[:,"R-Marrow"]),colon,float(dfSumEq.loc[:,"Lungs"]),float(dfSumEq.loc[:,"St-Wall"]),float(dfSumEq.loc[:,"UB-Wall"]),float(dfSumEq.loc[:,"Breasts"]),float(dfSumEq.loc[:,"Liver"]),float(dfSumEq.loc[:,"Thymus"]),float(dfSumEq.loc[:,"Thyroid"]),float(dfSumEq.loc[:,"Skin"]),float(dfSumEq.loc[:,"B-Surface"])])
            ...
            remainder_tissues = np.array([float(dfSumEq.loc[:,"Muscle"]),float(dfSumEq.loc[:,"Brain"]),float(dfSumEq.loc[:,"SI-Wall"]),float(dfSumEq.loc[:,"Kidneys"]),float(dfSumEq.loc[:,"Pancreas"]),float(dfSumEq.loc[:,"Spleen"]),float(dfSumEq.loc[:,"Thymus"]),float(dfSumEq.loc[:,"Uterus"]),float(dfSumEq.loc[:,"Adrenals"]),float(dfSumEq.loc[:,"ET-Region"])])
            remainder_masses = np.array([28000.,1400.,640.,310.,100.,180.,20.,80.,14.,15.])
